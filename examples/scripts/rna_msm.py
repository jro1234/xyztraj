...
def finalize_chi_vs_ONdist(nametag=''):
    ...
    plt.savefig("trajs-tica{}.png".format(nametag), dpi=600)
    plt.close()
...
for i,tica in enumerate(ticas):
    ...
    plt.savefig('tica-{}/tica-cc-landscape.png'.format(tica.lag))
    plt.close()
...
for i,clust in enumerate(clusts):
    ...
    plt.savefig('tica-{0}/timescales.png'.format(ticas[i].lag))
    plt.close()
...
for itsb in its:
    for msm in itsb.models:
        for ns in nstates:
            ...
            plt.savefig("tica-{0}/msm-{1}_lag-{2}_state.png".format(ticas[i].lag, msm.lag, ns))
            plt.close()
