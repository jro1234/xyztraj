...
class XYZReader(object):
    ...
    def readfile(self, xyzfilepath, nframes=None,
                 blocksize=None, stride=None):
        # TODO stride
        ...
        blocksize :: int number of frames to read between
          trajectory object updates
    ...
    def _readblocks(self, framekey, blocksize=None):
        ...
        while reading:
            ...
            try:
                ...
                firstline = next(self._file).split()  # noqa: F841
    ...
    def _build_iterator(self, framekey, nframes, blocksize):
        ...
        if blocksize is None:
            if nframes is None:
                ...
                iterblocks = lambda: iter([next(
                    self._readblocks(framekey, blocksize=nframes))])

        else:
            if nframes is None:
                # read to end in blocks of size blocksize
                iterblocks = lambda: iter(
                    self._readblocks(framekey, blocksize=blocksize))

            else:
                # read to nframes in blocks of size blocksize
                nblocks = bool(nframes % blocksize) + \
                    (nframes // blocksize)
